...
This test documents the behavior of Moon Cram executions in regards to backgrounding / detaching and open streams (STDOUT, STDERR, STDIN):
The `bash` process that executes a Moon Cram test expression will not close as long as any standard streams (`/dev/stdin`, `/dev/stdout` or `/dev/stderr`) are open.
...
Moon Cram waited for the whole 5 seconds
...
Bash process was not blocked by waiting for STD{ERR,OUT}, hence Moon Cram did not wait for the whole 5 seconds.
