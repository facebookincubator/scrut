...
Moon Cram supports appending of tests in front of a list of other files.

## Run test without append

```mooncram
$ "${MOON_CRAM_BIN}" test --work-directory "$(pwd)" --match-markdown "*.mdtest" "$TESTDIR/actual.mdtest"
...
```mooncram
$ "${MOON_CRAM_BIN}" test --work-directory "$(pwd)" --match-markdown "*.mdtest" --append-test-file-paths "$TESTDIR/append.mdtest" "$TESTDIR/actual.mdtest"
